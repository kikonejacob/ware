...
function envoyer_point_request(destID,caseID:integer):integer;
function Check_point_request():integer;
  ...
  begin
    ...
    begin
     ...
     begin
        ...
        if PartState=PS_PAUSE then
        waitPause;
        if timer_count=3 then  {reprise automatic}
        begin
        timerCmd:=-1;
        end;
        inc(timer_count);
...
begin
   ...
   p0_win.Caption:=Format('Parties remportées: %d',[gw.Players[0].winparts]);
   end
   else
   begin
   p1_point.Caption:=Inttostr(gw.Players[1].points);
   p1_win.Caption:=Format('Parties remportées: %d',[gw.Players[1].winparts]);
   end;
...
begin
  if ((gw.currplayer=0) and (CPU_busy=false)) then
  begin
    //showmessage('Jeu'+inttostr(msg.intarg));
    PlayCaseEx(msg.intarg,0,true);
    AffSelInfo(msg.intarg,0);
...
var
   ...
   my_res:PMYSQL_RES;
   data:PMYSQL_ROW;
   num,playerId:integer;
begin
   sql:=    ' SELECT * FROM ware_board';
   sql:=sql+' WHERE  destID='+_str(gw.players[1].Id);
   sql:=sql+' AND status=-1';
   sql:=sql+' AND cmd='+_str(LIVE_POINT_REQUEST);
   sql:=sql+' ORDER BY timestamp ASC';
   //showmessage(sql);
   my_res :=requete(sql);
   data := mysql_fetch_row(my_res);
   ...
   if (num>=gw.Players[playerId].caseBegin) and (num<=gw.Players[playerId].caseEnd) then
   begin
     sql:='UPDATE ware_board SET status=-1 WHERE id='+_str(msg.id);
     requete(sql);
      GetWarePoints(num,0,false)
   end ;
