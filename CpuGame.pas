...
function ComputerPlay(reflexion:integer):integer;
function TestPlay(num,PlayerId:integer;Cases:CaseArray;Points:integer):integer;
...
begin
  ...
  begin
    P0_p:=TestPlay(i,0,CaseArray(gw.cases),4);
    P1_p:=TestPlay(i,1,CaseArray(gw.cases),4);  {4 pour 4 magic}

    if (P0_P>=P1_P) and (P1_P=0) and (P0_P>0) then
    begin
        M0_C:=P0_P;
        M1_C:=P1_P;
        iNum:=i;
    ...
    if (P0_P>=P1_P) and (P1_P<M1_C)and (P0_P>0) then
    begin
        M0_C:=P0_P;
        M1_C:=P1_P;
        iNum:=i;
    ...
    if (P0_P>=P1_P) and (P1_P>=M0_C) and (P0_P>0) then
    begin
        M0_C:=P0_P;
        M1_C:=P1_P;
        iNum:=i;
...
{Teste toutes les cases}
function GetCaseFrom_Tactic3_1:integer;
var
  i,Inum,aNum:integer;
  P0_p,P1_p:integer; {point obtenu par P0,P1 pour Inum}
  M0_C,M1_C:integer;{point obtenu par P0,P1 pour i}
...
begin
  ...
  for i:= gw.Players[0].caseBegin to gw.Players[0].caseEnd  do
  begin
    P0_p:=TestPlay(i,0,CaseArray(gw.cases),3);
    P1_p:=TestPlay(i,1,CaseArray(gw.cases),4);  {4 pour 4 magic}

    if  (P0_P>=M0_C)  and (P1_P=0) and (P0_P>0) then
    begin
        M0_C:=P0_P;
        M1_C:=P1_P;
        iNum:=i;
    ...
    If (P0_P>=M0_C) and (P1_P<=M1_C) and (P0_P>0) then
    begin
        M0_C:=P0_P;
        M1_C:=P1_P;
        iNum:=i;
...
begin
   ...
   {Tactic 5}
   If reflexion>0 then{Niveau normale}
   begin
   ...
   form1.label1.Caption:='tactic5';
      ...
      If reflexion=2 then {Niveau difficile}
      begin
      j:=GetCaseFrom_Tactic3_1;
      If j>0 then
      begin
      iNum:=j;
      form1.label1.Caption:='tactic5.2';
      end;
      end;
   ...
   If iNum<>0 then
   begin
   Shortmsg(Format('l''ordi joue la case %d [%d pions]',[iNum,gw.Cases[iNum]]),0);
   AffSelInfo(iNum,0);
...
var
  Cases2:CaseArray;
  i,j,j3:integer;
  cend:integer; {la case ou sa tombe}
begin
  for i:=1 to 12 do
  Cases2[i]:=Cases[i];
  j:=0;
  ...
  begin
    ...
    If (Cases2[_cnv(i)]=points) then
    begin
      if IsPlayerCase(_cnv(i),PlayerId) and (i<>cend) then
      Inc(j);
      if  (i=cend) and (points=4)  then
      Inc(j);

      end;
    {pour le 4 case suppose que l'on va prendre la matière}
  ...
  j:=j+TestPlay(_cnv(cend),PlayerId,Cases2,points);
  end;
