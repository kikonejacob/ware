...
begin
  result:=0;
  CPU_busy:=true;
  if (IsPlayerCase(num,PlayerId)=false) and (CPU_busy=false) then
  begin
  Shortmsg('Ceci n''est pas votre case',1);
  result:=1;
  exit;
...
begin
  ...
  PlayCase(iNum,1,false);
  
  end
  else
  PlayCase(CurrCase,CurrPlayer,true)
end;
