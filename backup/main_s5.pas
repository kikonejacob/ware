...
GameInfo=record
   ...
   CasesFirst:array[1..12] of integer;
   FirstPlayer:integer;
   PartCoun:integer;
   Difficulty:integer;
   CurrCase:integer;
...
begin
  ...
  for i:=1 to 12  do
  begin
  GameFile.WriteBuffer(cases[i],Sizeof(integer))
  end;
  for i:=1 to 12  do
  begin
  GameFile.WriteBuffer(casesFirst[i],Sizeof(integer))
  end;
...
begin
  ...
  Gamefile:=TFileStream.Create('c:\jkgame.gcf',fmOpenRead or fmShareExclusive);
  Gamefile.Position:=0;
  for i:=1 to 12  do
  begin
  GameFile.ReadBuffer(cases[i],Sizeof(integer))
  end;
  for i:=1 to 12  do
  begin
  GameFile.ReadBuffer(casesFirst[i],Sizeof(integer))
  end;
...
var
   sr: TSearchRec;
begin
   if FindFirst('*.gth', faAnyFile, sr) = 0 then
   begin
      repeat
